<div align="center">
  <h2>👋 Hi, I'm jiang_I</h2>
  <p>NCU计科27届在读 | Java后端开发学习者</p>
</div>

---

### 📊 开发动态
<div align="center">
  <img src="https://github-readme-activity-graph.vercel.app/graph?username=34204002&theme=tokyo-night&hide_border=true&area=true&from=2025-09-01&to=2026-09-01" width="98%"/>
</div>

<div align="center">
  <img src="https://github-profile-summary-cards.vercel.app/api/cards/productive-time?username=34204002&theme=tokyonight&utcOffset=8" width="32%"/>
  <img src="https://github-readme-stats.vercel.app/api/top-langs/?username=34204002&layout=compact&hide_border=true&theme=tokyonight&langs_count=6&include_all_forks=true&hide=html,css,javascript" width="32%"/>
  <img src="https://github-readme-streak-stats.herokuapp.com?user=34204002&theme=tokyonight&hide_border=true" width="32%"/>
</div>

---

### 关于我
👋 你好呀，我是jiang_I，一名Java后端开发者，也在做AI相关的技术探索。

目前在后端岗位实习，日常和微服务、分布式打交道，做过图数据库的落地实践，一直对高并发相关技术很感兴趣；
同时也在持续探索AI和后端开发结合的更多可能，深耕大模型应用落地、RAG架构相关的实践。
写代码没什么特别的讲究，就是习惯规规矩矩，把该做的做好。

这里放的都是我的个人项目和技术实践，会不定期更新。

### 📬 找我可以戳这里
邮箱：34204002@qq.com
电话：18270519614
欢迎来聊技术，提建议都可以~

